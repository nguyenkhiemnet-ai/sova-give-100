...
export default function AuthModal({ isOpen, onClose, defaultTab = 'LOGIN' }: AuthModalProps) {
  ...
  // Hook PHẢI nằm trước tất cả các điều kiện early return để tránh lỗi sập màn hình React
  useEffect(() => {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    if (res.success) {
      ...
      const err = res.error || 'Email hoặc mật khẩu không chính xác.';
      // Kiểm tra xem tài khoản có phải tạo bằng Google không
      try {
        const chkRes = await fetch('/api/auth/check-method/', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ email: email.trim() })
        });
        const chkData = await chkRes.json();
        if (chkData?.isGoogleUser) {
          setErrorMessage("Tài khoản này được tạo bằng Google. Vui lòng bấm 'Tiếp tục với Google' bên dưới.");
          return;
        }
      } catch {}

      if (err.includes('Invalid login credentials') || err.includes('Email hoặc mật khẩu không chính xác')) {
        setErrorMessage("Email hoặc mật khẩu không chính xác. Nếu tài khoản được tạo bằng Google, vui lòng bấm 'Tiếp tục với Google' bên dưới.");
      } else {
        setErrorMessage(err);
      }
    }
    ...
    >
      ...
      >
        ...
        <div className="px-6 py-4 space-y-4">
          ...
          {errorMessage && (
            <div className="flex items-start gap-2 p-3 rounded-xl bg-rose-50 border border-rose-200 text-rose-800 text-xs font-medium animate-in fade-in">
              ...
              <div className="flex-1 space-y-2">
                ...
                {errorMessage.includes('Google') && (
                  <button
                    ...
                    className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-white border border-rose-300 hover:bg-rose-100 text-rose-900 font-bold text-xs shadow-2xs transition-all cursor-pointer"
                  >
                    ...
                    <span>Bấm vào đây để Tiếp tục với Google</span>
                  </button>
                )}
              </div>
            </div>
